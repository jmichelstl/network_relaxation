...
bool read_network(string file_name, double &offset, vector<double> &points, vector<edge_datum> &edges, double &min_y, double &max_y){
...
//    -false, otherwise
bool parse_batch_file(string file_name, string &base_name, string &tag, double *ks, double *kb, double fp_list[5], int *nmin, double *mass, double *fcut, int *nmax, strain_range &compr, strain_range &shearr){
    ...
    //Obtain the mechanical attributes of the network
    if(! read_line(input, nextline)) return false;
        
    num_read = sscanf(nextline.c_str(), "%lf %lf", ks, kb);
    if(num_read < 2 || *ks < 0 || *kb < 0){
...
__global__ void axial_energies(double ks, int npairs, double width, double *pos, unsigned char *fcodes, int *pair_list, double *lengths, char *wrap_codes, double *d_spe){
...
bool run_fire_md(int num_dof, int npairs, int ntriples, double width, double ks, double kb, double *d_pos, double *d_vel, double *d_forces, int *d_pair_list, double *d_lengths, char *d_e_offsets, int *d_triples, char *d_t_offsets, unsigned char* d_fcodes, double fire_params[5], double mass, double fcut, int max_steps, int nmin, dim3 e_grid, dim3 block, dim3 t_grid, double report_data[3], int &step_count){
    ...
    //energy.
    if(ks > 0){
        axial_energies<<<e_grid, block>>>(ks, npairs, width, d_pos, d_fcodes, d_pair_list, d_lengths, d_e_offsets, d_spe);
...
void do_relaxation_run(string net_file, string log_name, string edata_name, string disp_base, double ks, double kb, double fire_params[5], int nmin, double mass, double fcut, int n_steps, dim3 block, strain_range compr, strain_range shearr, bool strain_mode, bool itemize){

    //Points, edges, and bending triples
    vector<double> points, displaced_points;
    vector<edge_datum> edges;
    vector<triple_datum> triples;
    vector<unsigned char> fcodes;
    int iter, num_dof, steps, num_digits, disp_count = 1;
    double min_disp, max_disp, curr_disp, inc, smult, max_comp;
    bool success;

    //Data for carrying out calculations on the GPU
    double *d_pos, *d_forces, *d_vel, *d_lengths;
    int *d_pairs, *d_triples;
    ...
    //Minimum and maximum y coordinates for all points in a network
    double offset, miny, maxy, range;
    ...
    dim3 e_grid, t_grid, d_grid;

    //Means of collecting and reporting results
    double report_data[3];
    ...
    //of a network, and acquire bonds and triples
    read_network(net_file, offset, points, edges, miny, maxy);
    ...
    if(max_disp > 0){
        for(curr_disp = min_disp; curr_disp <= max_disp + FLT_TOL; curr_disp += inc){
            ...
            //Relax vertices
	    success = run_fire_md(num_dof, edges.size(), triples.size(), offset, ks, kb, d_pos, d_vel, d_forces, d_pairs, d_lengths, d_e_offsets, d_triples, d_t_offsets, d_fcodes, fire_params, mass, fcut, n_steps, nmin, e_grid, block, t_grid, report_data, steps);
	    ...
	    if(edata_file != NULL){
	        fprintf(edata_file, "%12.0lf\t%12.10lf\t%12.8le\t%12.8le\t%12.8le\n", 0., curr_disp, report_data[1], report_data[2], report_data[1]+report_data[2]);
	    }
        }
    }
    max_comp = max_disp > min_disp ? curr_disp - inc : max_disp;

    //Apply the desired combination shear displacements.
    ...
    if(max_disp > 0){
	    ...
	    printf("Steps: %d, RMS Force: %12.8le\n", steps, report_data[0]);
	    if(edata_file != NULL){
	        fprintf(edata_file, "%12.10lf\t%12.10lf\t%12.10le\t%12.10le\t%12.10le\n", curr_disp, max_comp, report_data[1], report_data[2], report_data[1]+report_data[2]);
	    }
...
void relaxation_batch(string batch_file, dim3 block, bool s_mode, bool itemize){
    ...
    if(!parse_batch_file(batch_file, base,tag,&ks,&kb,fp,&nmin,&mass,&fcut,&ncut,compr,shearr)){
        cerr << "Parsing of the batch file failed.\n";
        return;
    }

    //Generate lists of dat file and output file names
    code = get_file_lists(base, "dat", tag, net_files, log_files, disp_files, edat_files);
    ...
    for(iter = 0; iter < net_files.size(); iter++){
        do_relaxation_run(net_files[iter], log_files[iter], edat_files[iter], disp_files[iter], ks, kb, fp, nmin, mass, fcut, ncut, block, compr, shearr, s_mode, itemize);
    }
}
