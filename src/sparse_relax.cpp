 ...
 * conditions in the horizontal direction. A compression is applied, followed
 ...
 * the other. Next, a compression is applied, folowed by a shear displacement on
 * the top face of the network. A sparse QR solver is employed to solve for a 
 * zero-force configuaration of the network, given the applied bounary 
 * conditions.
 */
...
void find_null_space(cholmod_sparse *mat, SuiteSparse_long *null_dim, cholmod_sparse ***ns_proj, cholmod_common *cc){

    //Structure for storing the QR decomposition and null space of a matrix
    SuiteSparseQR_factorization <double> *QR;
    cholmod_sparse *top_spz, *right_spz, *spid, *null_id, *square_nid;
    cholmod_sparse *qxsnid, *N;
    int iter;
    long int liter;
    long int *rset, *cset;

    //Find the QR decomposition
    QR = SuiteSparseQR_factorize <double> (SPQR_ORDERING_DEFAULT, SPQR_DEFAULT_TOL, mat, cc);
    *null_dim = mat->nrow - QR->rank;
    //cout << "Null dimension: " << *null_dim << "\n";
    if(*null_dim == 0){
        SuiteSparseQR_free(&QR, cc);
	return;
    }

    //Construct a matrix of dimensions dim(mat) by null_dim, in which the
    //upper rank(mat) rows contain all zeros, while the bottom
    //null_dim by null_dim block is an identity matrix;
    top_spz = cholmod_l_spzeros(QR->rank, *null_dim, 0, CHOLMOD_REAL, cc);
    spid = cholmod_l_speye(*null_dim, *null_dim, CHOLMOD_REAL, cc);
    null_id = cholmod_l_vertcat(top_spz, spid, TRUE, cc);
    cholmod_l_free_sparse(&top_spz, cc);
    cholmod_l_free_sparse(&spid, cc);
    right_spz = cholmod_l_spzeros(mat->nrow, QR->rank, 0, CHOLMOD_REAL, cc);
    square_nid = cholmod_l_horzcat(null_id, right_spz, TRUE, cc);
    cholmod_l_free_sparse(&right_spz, cc);
    cholmod_l_free_sparse(&null_id, cc);
    //cholmod_l_print_sparse (square_nid, "Square NID", cc);
    
    //Compute the null space in matrix form
    
    //First multiply the padded matrix by the Q matrix. This is needed
    //because Q is stored in Householder form to save space, and
    //the qmult method requires a matrix with the same dimensions as Q.
    qxsnid = SuiteSparseQR_qmult(SPQR_QX, QR, square_nid, cc);
    //cholmod_l_print_sparse (qxsnid, "QXSNID", cc);
    SuiteSparseQR_free(&QR, cc);
    cholmod_l_free_sparse(&square_nid, cc);

    //Next, find the submatrix containing just the null space vectors.
    rset = (long int *) malloc(sizeof(long int) * mat->nrow);
    for(liter = 0; liter < mat->nrow; liter++) rset[liter] = liter;
    cset = (long int *) malloc(sizeof(long int) * *null_dim);
    for(liter = 0; liter < *null_dim; liter++) cset[liter] = liter;

    N = cholmod_l_submatrix(qxsnid,rset,mat->nrow, cset, *null_dim, 1, 0, cc);
    cholmod_l_free_sparse(&qxsnid, cc);
    free(cset);

    //Isolate each column of the null space matrix, so it can be individually
    //accessed
    *ns_proj = (cholmod_sparse **) malloc(sizeof(cholmod_sparse *) * *null_dim);
    for(liter = 0; liter < *null_dim; liter++){
        (*ns_proj)[liter] = cholmod_l_submatrix(N,rset,mat->nrow,&liter,1,1,0,cc);
    }

    //Destroy unneeded structures
    cholmod_l_free_sparse(&N, cc);
    free(rset);
}

//Prompt the user for a network to compress and shear, the mechanical
//attributes of the network, and the program of compression and shear that
//should be executed.
void do_relaxation_run(string dat_name, string log_name, string disp_base, string e_name, double ks, double kb, double mu, double min_c, double max_c, double c_inc, double min_s, double max_s, double s_inc, bool strain_mode, bool affine, bool remove_np, bool itemize, cholmod_common *cc){
    ...
    //Arrays to facilitate the computation of a network displacement field
    cholmod_dense *u, *u_final, *rhs, *bc_vec, *dprod;
    SuiteSparse_long null_dim, liter;

    //Sparse matrices for computing the psuedo-forces on the degrees of
    //freedom to be relaxed
    cholmod_sparse *kmat, *cond_mat, *mult, *prn, **ns_proj;
    ...
    if(remove_np){
        find_null_space(cond_mat, &null_dim, &ns_proj, cc);
	if(null_dim > 0) dprod = cholmod_l_zeros(1, 1, CHOLMOD_REAL, cc);
	if(log != NULL){
	    fprintf(log, "Null dimension: %ld\n", null_dim);
	}
    }
	...
	while(shear < max_s + FLT_TOL){
	    ...
	    if(remove_np){
                //For each vector in the null space basis, find its
		//projection along u, and remove this projection.
		for(liter = 0; iter < null_dim; liter ++){
                    cholmod_l_sdmult(ns_proj[liter],1,unity,null,u, dprod, cc);
		    cholmod_l_sdmult(ns_proj[liter],0,neg1,unity,dprod,u,cc);
		}
            }
	    ...
	    pe = e_ss + e_bend;
            if(efile != NULL){
	            ...
	            fprintf(efile, "%12.8lf\t%12.8lf\t%12.10le\t%12.10le\n", compression, shear, e_ss, e_bend);
		}
	    }
	    else{
                if(!itemize){ 
	            ...
	            printf("%12.8lf\t%12.8lf\t%12.10le\t%12.10le\n", compression, shear, e_ss, e_bend);
		}
	    }
            shear += s_inc;
    ...
    if(remove_np && null_dim > 0){
        for(liter = 0; liter < null_dim; liter++){
            cholmod_l_free_sparse(&(ns_proj[liter]), cc);
	}
	free(ns_proj);
	cholmod_l_free_dense(&dprod, cc);
    }
    cholmod_l_finish(cc);
...
void run_relaxation_batch(string file_name, bool use_gpu, bool strain_mode, bool affine, bool remove_np, bool itemize){
    ...
    code = get_file_lists(base, "dat", tag, dat_files, log_files, disp_files, edat_files);
    ...
    for(iter = 0; iter < dat_files.size(); iter++){
        do_relaxation_run(dat_files[iter], log_files[iter], disp_files[iter], edat_files[iter], ks, kb, mu, min_c,  max_c, c_inc, min_s, max_s, s_inc, strain_mode, affine, remove_np, itemize, &common);
    }
...
void run_relaxation_interactive(bool use_gpu, bool strain_mode, bool affine, bool remove_np, bool itemize){
    ...
    //Perform network relaxation
    do_relaxation_run(dat_name, log_name, disp_base, e_name, ks, kb, mu, min_c,  max_c, c_inc, min_s, max_s, s_inc, strain_mode, affine, remove_np, itemize, &common);
...
int main(int argc, char **argv){
    ...
    bool remove_np = false, itemize = false;
    ...
    //usage and exit.
    while((c = getopt(argc, argv, "b:ginsz")) != -1){
	switch(c) {
	    case 'b':
	...
	free(batch_file);
	run_relaxation_batch(bstring, use_gpu, strain_mode, affine, remove_np, itemize);
    }
    else{
        run_relaxation_interactive(use_gpu, strain_mode, affine, remove_np, itemize);
    }
